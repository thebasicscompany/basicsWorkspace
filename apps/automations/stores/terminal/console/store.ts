...
import { useExecutionStore } from '@/apps/automations/stores/execution'
import { useNotificationStore } from '@/apps/automations/stores/notifications'
import { indexedDBStorage } from '@/apps/automations/stores/terminal/console/storage'
import type { ConsoleEntry, ConsoleStore, ConsoleUpdate } from '@/apps/automations/stores/terminal/console/types'
 ...
 */
const notifyBlockError = ({ error, blockName, workflowId, logContext: _logContext }: NotifyBlockErrorParams) => {
  // Stub: general settings / error notification preferences not yet wired
  const isErrorNotificationsEnabled = true
  ...
  try {
    ...
    console.error('[TerminalConsoleStore] Failed to create block error notification', notificationError)
  }
}
    ...
    persist(
      ...
      }),
      {
        ...
        onRehydrateStorage: () => (_state, error) => {
          if (error) {
            console.error('[TerminalConsoleStore] Failed to rehydrate console store', error)
          }
        },
