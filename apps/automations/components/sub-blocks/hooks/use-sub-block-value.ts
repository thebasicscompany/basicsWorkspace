...
import { useWorkflowDiffStore } from '@/apps/automations/stores/workflow-diff'
...
export function useSubBlockValue<T = any>(
  ...
  const blockType = useWorkflowStore(
    useCallback((state: any) => state.blocks?.[blockId]?.type, [blockId])
  )

  const initialValue = useWorkflowStore(
    useCallback(
      (state: any) => state.blocks?.[blockId]?.subBlocks?.[subBlockId]?.value ?? null,
      [blockId, subBlockId]
    )
  )
  ...
  const modelSubBlockValue = useStoreWithEqualityFn(
    useSubBlockStore,
    useCallback((state: any) => (blockId ? state.getValue(blockId, 'model') : null), [blockId]),
    (a: any, b: any) => a === b
  )
