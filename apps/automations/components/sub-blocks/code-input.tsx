...
} from '@/apps/automations/lib/sanitization-references'
import {
  checkEnvVarTrigger,
  EnvVarDropdown,
} from '@/apps/automations/components/sub-blocks/env-var-dropdown'
import {
  checkTagTrigger,
  TagDropdown,
} from '@/apps/automations/components/sub-blocks/tag-dropdown'
...
import { restoreCursorAfterInsertion } from '@/apps/automations/components/sub-blocks/stubs/editor-utils'
import { WandPromptBar } from '@/apps/automations/components/sub-blocks/stubs/wand-prompt-bar'
...
import { createShouldHighlightEnvVar, useAvailableEnvVarKeys } from '@/hooks/stubs/use-available-env-vars'
import { useCodeUndoRedo } from '@/hooks/stubs/use-code-undo-redo'
    ...
    placeholders.forEach(({ placeholder, original, type }) => {
      if (type === 'env') {
        highlightedCode = highlightedCode.replace(
          placeholder,
          `<span style="color: var(--color-accent);">${original}</span>`
        )
      } else if (type === 'var') {
        const escaped = original.replace(/</g, '&lt;').replace(/>/g, '&gt;')
        highlightedCode = highlightedCode.replace(
          placeholder,
          `<span style="color: var(--color-accent);">${escaped}</span>`
        )
      }
    })
...
export const Code = memo(function Code({
  ...
  return (
    <>
      ...
      <CodeEditor.Container onDragOver={(e: any) => e.preventDefault()} onDrop={handleDrop}>
