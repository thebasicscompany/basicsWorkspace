...
// ─── Caret position helper (copied from Sim) ───────────────────────────────
 ...
 * for selection in input fields. Uses a portal-based dropdown for consistent styling.
 */
export const EnvVarDropdown: React.FC<EnvVarDropdownProps> = ({
  ...
  // Stubbed environment hooks — return empty until we build env var management
  ...
  const dropdownMaxHeight = maxHeight !== 'none' ? 192 : 400
  ...
  const dropdown = (
    ...
    >
      <div
        className="min-w-[280px] rounded-lg border bg-white shadow-md"
        style={{
      ...
      >
        <ScrollArea style={{ maxHeight: `${dropdownMaxHeight}px` }}>
          {filteredEnvVars.length === 0 ? (
            <button
              className="flex w-full items-center gap-2 px-3 py-2 text-xs hover:bg-[var(--color-bg-base)] cursor-pointer"
                ...
                {group.variables.map((envVar) => {
                  ...
                  return (
                    <button
                      ...
                      className="flex w-full items-center px-3 py-1.5 text-xs cursor-pointer"
                      style={{
