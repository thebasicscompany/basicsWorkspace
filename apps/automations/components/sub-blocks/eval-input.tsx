...
import { Button, Input, Textarea, Tooltip } from '@/apps/automations/components/sub-blocks/stubs/emcn'
...
export function EvalInput({
  ...
  const renderMetricHeader = (metric: EvalMetric, index: number) => (
    <div className='flex items-center justify-between overflow-hidden rounded-t-[4px] border-[var(--color-border)] border-b bg-[var(--color-bg-base)] px-[10px] py-[5px]'>
    ...
    <div className='space-y-[8px]'>
      {metrics.map((metric, index) => (
        <div
          ...
          className='group relative overflow-visible rounded-[4px] border border-[var(--color-border)]'
        >
          {renderMetricHeader(metric, index)}

          <div className='flex flex-col gap-[8px] border-[var(--color-border)] px-[10px] pt-[6px] pb-[10px]'>
            <div key={`name-${metric.id}`} className='flex flex-col gap-[6px]'>
              <Label className='text-[13px]'>Name</Label>
              <Input
                name='name'
                value={metric.name}
                onChange={(e: any) => updateMetric(metric.id, 'name', e.target.value)}
            ...
            <div key={`range-${metric.id}`} className='grid grid-cols-2 gap-[8px]'>
              <div className='flex flex-col gap-[6px]'>
                ...
                <Input
                  ...
                  onChange={(e: any) => updateRange(metric.id, 'min', e.target.value)}
                  onBlur={(e: any) => handleRangeBlur(metric.id, 'min', e.target.value)}
                  disabled={isPreview || disabled}
                  autoComplete='off'
              ...
              <div className='flex flex-col gap-[6px]'>
                ...
                <Input
                  ...
                  onChange={(e: any) => updateRange(metric.id, 'max', e.target.value)}
                  onBlur={(e: any) => handleRangeBlur(metric.id, 'max', e.target.value)}
                  disabled={isPreview || disabled}
                  autoComplete='off'
                  data-form-type='other'
                  name='eval-range-max'
