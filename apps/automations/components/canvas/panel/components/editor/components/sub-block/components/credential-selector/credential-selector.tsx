...
import { OAuthRequiredModal } from '@/app/workspace/[orgId]/w/[workflowId]/components/panel/components/editor/components/sub-block/components/credential-selector/components/oauth-required-modal'
...
import { CREDENTIAL_SET } from '@/lib/sim/executor/constants'
...
export function CredentialSelector({
  ...
  useEffect(() => {
    if (!selectedId || selectedCredential || credentialsLoading || !orgId) {
    ...
    ;(async () => {
      try {
        const response = await fetch(
          `/api/credentials?orgId=${encodeURIComponent(orgId)}&credentialId=${encodeURIComponent(selectedId)}`
        )
  ...
  }, [selectedId, selectedCredential, credentialsLoading, orgId])
  ...
  useCredentialRefreshTriggers(refetchCredentials, effectiveProviderId, orgId)
  ...
  }, [orgId, effectiveProviderId, serviceId, activeWorkflowId])
