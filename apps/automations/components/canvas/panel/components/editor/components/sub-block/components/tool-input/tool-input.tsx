...
import { useUserPermissionsContext } from '@/app/workspace/[orgId]/providers/workspace-permissions-provider'
import {
  LongInput,
  ShortInput,
} from '@/app/workspace/[orgId]/w/[workflowId]/components/panel/components/editor/components/sub-block/components'
import {
  type CustomTool,
  CustomToolModal,
} from '@/app/workspace/[orgId]/w/[workflowId]/components/panel/components/editor/components/sub-block/components/tool-input/components/custom-tool-modal/custom-tool-modal'
import { ToolCredentialSelector } from '@/app/workspace/[orgId]/w/[workflowId]/components/panel/components/editor/components/sub-block/components/tool-input/components/tools/credential-selector'
import { ParameterWithLabel } from '@/app/workspace/[orgId]/w/[workflowId]/components/panel/components/editor/components/sub-block/components/tool-input/components/tools/parameter'
import { ToolSubBlockRenderer } from '@/app/workspace/[orgId]/w/[workflowId]/components/panel/components/editor/components/sub-block/components/tool-input/components/tools/sub-block-renderer'
import type { StoredTool } from '@/app/workspace/[orgId]/w/[workflowId]/components/panel/components/editor/components/sub-block/components/tool-input/types'
import {
  isCustomToolAlreadySelected,
  isMcpToolAlreadySelected,
  isWorkflowAlreadySelected,
} from '@/app/workspace/[orgId]/w/[workflowId]/components/panel/components/editor/components/sub-block/components/tool-input/utils'
import { useSubBlockValue } from '@/app/workspace/[orgId]/w/[workflowId]/components/panel/components/editor/components/sub-block/hooks/use-sub-block-value'
import type { WandControlHandlers } from '@/app/workspace/[orgId]/w/[workflowId]/components/panel/components/editor/components/sub-block/sub-block'
...
import { getProviderFromModel, supportsToolUsageControl } from '@/lib/sim/providers/utils'
...
} from '@/lib/sim/tools/params-resolver'
...
export const ToolInput = memo(function ToolInput({
  ...
  const { data: customTools = [] } = useCustomTools(shouldFetchCustomTools ? orgId : '')

  const { mcpTools, isLoading: mcpLoading } = useMcpTools(orgId)

  const { data: mcpServers = [], isLoading: mcpServersLoading } = useMcpServers(orgId)
  const { data: storedMcpTools = [] } = useStoredMcpTools(orgId)
  const forceRefreshMcpTools = useForceRefreshMcpTools()
  useMcpToolsEvents(orgId)
  const { navigateToSettings } = useSettingsNavigation()
  const mcpDataLoading = mcpLoading || mcpServersLoading

  const { data: workflowsList = [] } = useWorkflows(orgId, { syncRegistry: false })
  ...
  useEffect(() => {
    ...
    if (hasMcpTools && !hasRefreshedRef.current) {
      ...
      forceRefreshMcpTools(orgId)
    }
  }, [hasMcpTools, forceRefreshMcpTools, orgId, isPreview])
