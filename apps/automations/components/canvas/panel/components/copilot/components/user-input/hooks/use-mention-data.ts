...
export function useMentionData(props: UseMentionDataProps): MentionDataReturn {
  ...
  const ensureKnowledgeLoaded = useCallback(async () => {
    ...
    try {
      ...
      const resp = await fetch(`/api/knowledge?orgId=${orgId}`)
  ...
  }, [isLoadingKnowledge, knowledgeBases.length, orgId])
  ...
  const ensureLogsLoaded = useCallback(async () => {
    ...
    try {
      ...
      const resp = await fetch(`/api/logs?orgId=${orgId}&limit=50&details=full`)
  ...
  }, [isLoadingLogs, logsList.length, orgId])
