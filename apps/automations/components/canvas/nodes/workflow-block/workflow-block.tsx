...
import { Badge, Tooltip } from '@/lib/core/utils/cn'
...
import { ActionBar } from '@/app/workspace/[orgId]/w/[workflowId]/components/action-bar/action-bar'
import {
  useBlockProperties,
  useChildWorkflow,
  useWebhookInfo,
} from '@/app/workspace/[orgId]/w/[workflowId]/components/workflow-block/hooks'
import type { WorkflowBlockProps } from '@/app/workspace/[orgId]/w/[workflowId]/components/workflow-block/types'
import {
  getProviderName,
  shouldSkipBlockRender,
} from '@/app/workspace/[orgId]/w/[workflowId]/components/workflow-block/utils'
import { useBlockVisual } from '@/app/workspace/[orgId]/w/[workflowId]/hooks'
import { useBlockDimensions } from '@/app/workspace/[orgId]/w/[workflowId]/hooks/use-block-dimensions'
import { getBlock } from '@/blocks'
import { SELECTOR_TYPES_HYDRATION_REQUIRED, type SubBlockConfig } from '@/lib/sim/blocks/types'
...
import { useVariablesStore } from '@/apps/automations/stores/panel'
...
import { wouldCreateCycle } from '@/apps/automations/stores/workflow-utils'
...
const SubBlockRow = memo(function SubBlockRow({
  ...
  const { data: mcpServers = [] } = useMcpServers(orgId || '')
  ...
  const { data: mcpToolsData = [] } = useMcpToolsQuery(orgId || '')
  ...
  const { data: tables = [] } = useTablesList(orgId || '')
   ...
   */
  const { data: customTools = [] } = useCustomTools(orgId || '')
  ...
  }, [subBlock?.type, rawValue, customTools, orgId])
   ...
   */
  const { data: workspaceSkills = [] } = useSkills(orgId || '')
