...
import {
  ...
  GitFork as SplitIcon,
  XCircle as XCircleIcon,
} from '@phosphor-icons/react'
import { getBlock } from '@/lib/sim/blocks/registry'
import { isWorkflowBlockType } from '@/lib/sim/executor/constants'
import { TERMINAL_BLOCK_COLUMN_WIDTH } from '@/apps/automations/stores/constants'
...
export function getBlockIcon(
  ...
  if (blockType === 'parallel') {
    return SplitIcon as any
  }

  if (blockType === 'workflow') {
    return NetworkIcon as any
  }

  if (blockType === 'error') {
    return XCircleIcon as any
  }
...
export const TERMINAL_CONFIG = {
  ...
  HEADER_TEXT_CLASS: 'font-normal text-[var(--color-text-secondary)] text-[13px]',
} as const
