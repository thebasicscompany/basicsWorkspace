/**
 * Edit workflow types — adapted from Sim's lib/copilot/tools/server/workflow/edit-workflow/types.ts
 * Changes: removed @sim/logger import (use console), removed PermissionGroupConfig (stubbed as null)
 */
...
export function logSkippedItem(skippedItems: SkippedItem[], item: SkippedItem): void {
  console.warn(`[EditWorkflow] Skipped ${item.operationType} operation: ${item.reason}`, {
    type: item.type,
...
/**
 * Permission group config stub — we don't have permission groups yet
 */
