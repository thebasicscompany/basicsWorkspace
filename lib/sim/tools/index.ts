...
import { resolveSkillContent } from '@/lib/sim/executor/handlers/agent/skills-resolver'
import type { ExecutionContext } from '@/lib/sim/executor/types'
import type { ErrorInfo } from '@/lib/sim/tools/error-extractors'
import { extractErrorMessage } from '@/lib/sim/tools/error-extractors'
import type {
  BYOKProviderId,
  OAuthTokenPayload,
  ToolConfig,
  ...
  ToolResponse,
  ToolRetryConfig,
} from '@/lib/sim/tools/types'
import {
  formatRequestParams,
  getTool,
  getToolAsync,
  validateRequiredParametersAfterMerge,
} from '@/lib/sim/tools/utils'
...
async function processFileOutputs(
  ...
  try {
    ...
    const { FileToolProcessor } = await import('@/lib/sim/executor/utils/file-tool-processor')
      ...
      try {
        if (isInternalRoute) {
          ...
          const secureResponse = await secureFetchWithPinnedIP(fullUrl, urlValidation.resolvedIP!, {
            ...
            // @ts-ignore — timeout is a non-standard but supported RequestInit extension
