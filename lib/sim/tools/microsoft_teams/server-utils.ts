 ...
 */
import type { Logger } from '@/lib/sim/logger'
import { secureFetchWithValidation } from '@/lib/core/security/input-validation.server'
import { processFilesToUserFiles, type RawFileInput } from '@/lib/uploads/utils/file-utils'
import { downloadFileFromStorage } from '@/lib/uploads/utils/file-utils.server'
import type { UserFile } from '@/lib/sim/executor/types'
import type { GraphApiErrorResponse, GraphDriveItem } from '@/lib/sim/tools/microsoft_teams/types'
...
export async function uploadFilesForTeamsMessage(params: {
  ...
  // @ts-ignore — processFilesToUserFiles returns Promise but cast handles Phase 4 runtime type
  const userFiles = processFilesToUserFiles(rawFiles, requestId, log) as unknown as UserFile[]
  ...
  for (const file of userFiles) {
    ...
    const uploadResponse = await secureFetchWithValidation(
      uploadUrl,
      {
        ...
        // @ts-ignore — Buffer is valid BodyInit at runtime in Node.js
        body: buffer,
      },
      'uploadUrl'
    )
