...
import { MEMORY } from '@/lib/sim/executor/constants'
...
import { PROVIDER_DEFINITIONS } from '@/lib/sim/providers/models'
...
export class Memory {
  ...
  private async fetchMemory(orgId: string, key: string): Promise<Message[]> {
    ...
    return data.filter(
      // @ts-ignore — msg is unknown[] element; runtime check is correct
  ...
  private async appendMessage(orgId: string, key: string, message: Message): Promise<void> {
    ...
    await db
      ...
      .onConflictDoUpdate({
        target: [memory.orgId, memory.key],
        set: {
