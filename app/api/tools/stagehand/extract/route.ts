...
const logger = { info: (...args: any[]) => console.log('[stagehand-extract]', ...args), warn: (...args: any[]) => console.warn('[stagehand-extract]', ...args), error: (...args: any[]) => console.error('[stagehand-extract]', ...args) }

// Type from @browserbasehq/stagehand — package is optional, loaded dynamically at runtime
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      const { Stagehand } = await import(/* webpackIgnore: true */ pkg)
      ...
      stagehand = new Stagehand({
        ...
        verbose: 1,
        logger: (msg: unknown) => logger.info(typeof msg === 'string' ? msg : JSON.stringify(msg)),
        model: {
          modelName,
          apiKey: apiKey,
        },
