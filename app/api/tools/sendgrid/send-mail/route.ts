...
const logger = { info: (...args: any[]) => console.log('[sendgrid-send-mail]', ...args), warn: (...args: any[]) => console.warn('[sendgrid-send-mail]', ...args), error: (...args: any[]) => console.error('[sendgrid-send-mail]', ...args) }
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (validatedData.attachments && validatedData.attachments.length > 0) {
      ...
      const userFiles = processFilesToUserFiles(rawAttachments, requestId, logger) as Array<{ name: string; type?: string; size: number }>
