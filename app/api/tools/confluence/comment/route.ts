...
const logger = { info: (...args: any[]) => console.log('[confluence-comment]', ...args), warn: (...args: any[]) => console.warn('[confluence-comment]', ...args), error: (...args: any[]) => console.error('[confluence-comment]', ...args) }
...
const putCommentSchema = z
  ...
  .refine(
    (data) => validateAlphanumericId(data.commentId, 'commentId', 255).isValid,
    { message: 'Invalid comment ID', path: ['commentId'] }
  )
