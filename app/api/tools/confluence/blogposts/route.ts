...
const logger = { info: (...args: any[]) => console.log('[confluence-blogposts]', ...args), warn: (...args: any[]) => console.warn('[confluence-blogposts]', ...args), error: (...args: any[]) => console.error('[confluence-blogposts]', ...args) }
...
const getBlogPostSchema = z
  ...
  .refine(
    (data) => validateAlphanumericId(data.blogPostId, 'blogPostId', 255).isValid,
    { message: 'Invalid blog post ID', path: ['blogPostId'] }
  )
