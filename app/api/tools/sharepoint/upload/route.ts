...
import type { MicrosoftGraphDriveItem } from '@/lib/sim/tools/onedrive/types'

export const dynamic = 'force-dynamic'

const logger = { info: (...args: any[]) => console.log('[sharepoint-upload]', ...args), warn: (...args: any[]) => console.warn('[sharepoint-upload]', ...args), error: (...args: any[]) => console.error('[sharepoint-upload]', ...args) }
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const userFiles = processFilesToUserFiles(validatedData.files, requestId, logger) as Array<{ name: string; type?: string; size: number }>
    ...
    for (const userFile of userFiles) {
      ...
      const buffer = new Uint8Array(await downloadFileFromStorage(userFile, requestId, logger))
