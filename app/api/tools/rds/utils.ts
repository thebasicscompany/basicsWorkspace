...
async function introspectPostgresql(
  ...
  const schemasResult = await executeStatement(
    client, resourceArn, secretArn, database,
  ...
  for (const tableRow of tablesResult.rows) {
    ...
    const columnsResult = await executeStatement(
      client, resourceArn, secretArn, database,
      `SELECT c.column_name, c.data_type, c.is_nullable, c.column_default, c.udt_name
    ...
    const pkResult = await executeStatement(
      ...
      `SELECT kcu.column_name
       ...
       WHERE tc.constraint_type = 'PRIMARY KEY'
         AND tc.table_schema = :tableSchema AND tc.table_name = :tableName`,
      [
        { name: 'tableSchema', value: { stringValue: tableSchema } },
        { name: 'tableName', value: { stringValue: tableName } },
      ]
    ...
    const foreignKeys = fkResult.rows.map((r) => {
      const fkRow = r as { column_name: string; foreign_table_name: string; foreign_column_name: string }
      return { column: fkRow.column_name, referencesTable: fkRow.foreign_table_name, referencesColumn: fkRow.foreign_column_name }
    })
    ...
    const indexesResult = await executeStatement(
      ...
      `SELECT i.relname AS index_name, a.attname AS column_name, ix.indisunique AS is_unique
       ...
       WHERE t.relkind = 'r' AND n.nspname = :tableSchema AND t.relname = :tableName AND NOT ix.indisprimary
    ...
    for (const idxRow of indexesResult.rows) {
      ...
      if (!indexMap.has(idx.index_name)) {
        indexMap.set(idx.index_name, { name: idx.index_name, columns: [], unique: idx.is_unique })
      }
    ...
    const columns = columnsResult.rows.map((colRow) => {
      const col = colRow as { column_name: string; data_type: string; is_nullable: string; column_default: string | null; udt_name: string }
      ...
      return {
        ...
        ...(fk && { references: { table: fk.referencesTable, column: fk.referencesColumn } }),
      }
    })

    tables.push({ name: tableName, schema: tableSchema, columns, primaryKey: primaryKeyColumns, foreignKeys, indexes })
  }
  ...
  for (const tableRow of tablesResult.rows) {
    ...
    const columnsResult = await executeStatement(
      ...
      `SELECT COLUMN_NAME as column_name, DATA_TYPE as data_type, IS_NULLABLE as is_nullable,
              COLUMN_DEFAULT as column_default, COLUMN_TYPE as column_type, COLUMN_KEY as column_key
    ...
    const pkResult = await executeStatement(
      ...
      `SELECT COLUMN_NAME as column_name FROM information_schema.KEY_COLUMN_USAGE
       WHERE TABLE_SCHEMA = :tableSchema AND TABLE_NAME = :tableName AND CONSTRAINT_NAME = 'PRIMARY'
       ORDER BY ORDINAL_POSITION`,
      [
        { name: 'tableSchema', value: { stringValue: tableSchema } },
        { name: 'tableName', value: { stringValue: tableName } },
    ...
    const fkResult = await executeStatement(
      client, resourceArn, secretArn, database,
      `SELECT kcu.COLUMN_NAME as column_name, kcu.REFERENCED_TABLE_NAME as foreign_table_name,
              kcu.REFERENCED_COLUMN_NAME as foreign_column_name
       FROM information_schema.KEY_COLUMN_USAGE kcu
       WHERE kcu.TABLE_SCHEMA = :tableSchema AND kcu.TABLE_NAME = :tableName AND kcu.REFERENCED_TABLE_NAME IS NOT NULL`,
      [
        { name: 'tableSchema', value: { stringValue: tableSchema } },
        { name: 'tableName', value: { stringValue: tableName } },
      ]
    ...
    for (const idxRow of indexesResult.rows) {
      ...
      if (!indexMap.has(idx.index_name)) {
        indexMap.set(idx.index_name, { name: idx.index_name, columns: [], unique: idx.non_unique === 0 })
      }
      indexMap.get(idx.index_name)!.columns.push(idx.column_name)
    }
    const indexes = Array.from(indexMap.values())
    ...
    const columns = columnsResult.rows.map((colRow) => {
      const col = colRow as { column_name: string; data_type: string; is_nullable: string; column_default: string | null; column_type: string; column_key: string }
      const columnName = col.column_name
      const fk = foreignKeys.find((f) => f.column === columnName)
      return {
        name: columnName,
