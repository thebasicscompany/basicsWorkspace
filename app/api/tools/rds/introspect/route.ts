...
const logger = { info: (...args: any[]) => console.log('[RDSIntrospectAPI]', ...args), warn: (...args: any[]) => console.warn('[RDSIntrospectAPI]', ...args), error: (...args: any[]) => console.error('[RDSIntrospectAPI]', ...args) }
  ...
  try {
    ...
    try {
      const result = await executeIntrospect(
        client, params.resourceArn, params.secretArn, params.database,
        params.schema, params.engine as RdsEngine | undefined
      )
