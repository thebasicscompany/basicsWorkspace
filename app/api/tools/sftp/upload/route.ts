...
const logger = { info: (...args: any[]) => console.log('[sftp-upload]', ...args), warn: (...args: any[]) => console.warn('[sftp-upload]', ...args), error: (...args: any[]) => console.error('[sftp-upload]', ...args) }
  ...
  try {
    ...
    try {
      ...
      if (hasFiles) {
        ...
        const userFiles = processFilesToUserFiles(rawFiles, requestId, logger) as Array<{ name: string; type?: string; size: number }>
