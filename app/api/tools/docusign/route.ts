...
const logger = { info: (...args: any[]) => console.log('[docusign]', ...args), warn: (...args: any[]) => console.warn('[docusign]', ...args), error: (...args: any[]) => console.error('[docusign]', ...args) }
...
async function handleSendEnvelope(
  ...
  if (file) {
    try {
      ...
      const userFiles = processFilesToUserFiles([parsed as unknown as RawFileInput], 'docusign-send', logger) as Array<{ name: string; type?: string; size: number }>
